...
contract Beccamose is ERC721, ERC721URIStorage, Ownable {
    ...
    function safeMint(address to, address address1, address address2)
        public
        onlyOwner
    {   
        ...
        returns (string memory)
    {
        ...
        string memory svg = string(abi.encodePacked(
            ...
            '<text x="50%" y="55%" font-size="10" class="base" dominant-baseline="middle" text-anchor="middle">', addressToString(metadata.address1), '</text>',
            ...
            '<text x="50%" y="72%" font-size="10" class="base" dominant-baseline="middle" text-anchor="middle">', addressToString(metadata.address2), '</text>',
            '</svg>'
        ));

        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Token ', toString(tokenId), '", "description": "Se semo beccati", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(svg)), '", "attributes": [{"trait_type": "address1", "value": "', addressToString(metadata.address1), '"}, {"trait_type": "address2", "value": "', addressToString(metadata.address2), '"}]}'))));
